...
import sys
import copy
sys.path.append("/nlp_group/huangwei12/Mul_graph/MKGformer/MKG/models_tu_diffusion_cls_de_jian_jie")
from deffusion_model import linear_beta_schedule,quadratic_beta_schedule,cosine_beta_schedule,exponential_beta_schedule
from deffusion_model import forward_diffusion_process,reverse_diffusion_process
from deffusion_model import CFDenoiser
...
class UnimoEncoder(nn.Module):
    ...
    def forward(
        ...
        return BaseModelOutput(
            last_hidden_state=text_hidden_states,hidden_states=vision_hidden_states, attentions=all_text_attentions
        )
...
class UnimoModel(nn.Module):
    ...
    def forward(
        ...
        image_sequence_output = encoder_outputs[1]
        image_sequence_output, _ = image_sequence_output.max(dim=1)
        ...
        return BaseModelOutputWithPooling(
            ...
            hidden_states=image_sequence_output,
...
class UnimoForMaskedLM(nn.Module):
    def __init__(self, vision_config, text_config,timesteps,diffusion_block):
        super().__init__()
        self.unimo = UnimoModel(vision_config, text_config)
        self.cls = UnimoOnlyMLMHead(text_config)
        self.config = text_config
        self.timesteps = timesteps
        self.gat_net = GCNEncoder(768,1536,768,3)
        # 这里有一组对比实验
        self.beta_schedule = linear_beta_schedule(timesteps)
        # self.beta_schedule = cosine_beta_schedule(timesteps)
        self.denoiser = CFDenoiser(1024,2048,1536,diffusion_block,timesteps)
        self.tie_weights()
    ...
    def forward(
        ...
        image_sequence_output = outputs[2]
        ...
        epsilon = 1e-8
        mask_prediction_scores = prediction_scores[:,30522:45473]
        
        diffusion_prediction_scores = mask_prediction_scores.clone()
        diffusion_prediction_scores_index = mask_prediction_scores.clone().detach()
        _, indices = torch.topk(diffusion_prediction_scores_index, 1024, dim=-1)
        sorted_indices = torch.sort(indices, dim=1).values.detach()
        mask_diffusion_prediction_scores = torch.gather(diffusion_prediction_scores, 1, sorted_indices) 
        mask_diffusion_prediction_scores_loss_true = mask_diffusion_prediction_scores.clone()
        mask_diffusion_prediction_scores_loss_true = F.softmax(mask_diffusion_prediction_scores_loss_true, dim=-1).clamp(min=epsilon).detach()


        noised_embeddings = forward_diffusion_process(mask_diffusion_prediction_scores,self.timesteps,self.beta_schedule)
        noised_embedding = noised_embeddings[-1]
        mask_sequence_output = torch.cat((mask_sequence_output,image_sequence_output),dim=-1)
        # import pdb;pdb.set_trace()
        mask_diffusion_prediction_scores = reverse_diffusion_process(self.denoiser,noised_embedding,mask_sequence_output,self.timesteps)
        mask_diffusion_prediction_scores_loss = mask_diffusion_prediction_scores.clone()
        mask_diffusion_prediction_scores_loss = F.softmax(mask_diffusion_prediction_scores_loss, dim=-1).clamp(min=epsilon)
        kl_loss = F.kl_div(mask_diffusion_prediction_scores_loss.log(), mask_diffusion_prediction_scores_loss_true, reduction='batchmean')
        # import pdb;pdb.set_trace()
        # kl_loss = F.mse_loss(mask_diffusion_prediction_scores_loss.log(), mask_diffusion_prediction_scores_loss_true,reduction='mean')


        # diffusion_prediction_scores.scatter_(1, sorted_indices, mask_diffusion_prediction_scores)

        # prediction_scores_sof = mask_prediction_scores_sof* (1 - mask_prediction_scores_sof)+diffusion_prediction_scores*mask_prediction_scores_sof
        diffusion_prediction_scores = diffusion_prediction_scores.scatter(1, sorted_indices, mask_diffusion_prediction_scores)
        prediction_scores = diffusion_prediction_scores
